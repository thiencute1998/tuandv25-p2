...
@section('main-content-inner')
            ...
            <div class="col-lg-12 col-ml-12">
                <div class="row">
                    ...
                    <div class="col-12 mt-5">
                        <div class="card">
                            <div class="card-body">
                                <form id="product-form" name="product-form" action="{{ route('slides-update', ['id'=> $slide->id]) }}" method="POST" enctype="multipart/form-data" class="dropzone border-0">
                                    @csrf
                                    @if (session('edit-success'))
                                        <h5 class="slide-message mb-2 text-success">{{ session('edit-success') }}</h5>
                                    @endif
                                    <h4 class="header-title product-add-title">Edit slide</h4>
                                    <div class="form-group">
                                        <label for="product-name" class="col-form-label">Content</label>
                                        <input class="form-control" name="content" type="text" value="{{ $slide->content }}" >
                                    </div>
                                    <div class="form-group">
                                        <label for="services" class="col-form-label">Status</label>
                                        <select class="custom-select slide-status" name="status" data-value="{{ $slide->status }}">
                                                ...
                                                <div class="d-flex">
                                                    <div class="mb-3 mr-2 image-item" style="position: relative">
                                                        ...
                                                        <img width="1600px" height="550px" class="slide-img"
                                                             src="{{asset('upload/admin/banner/slide/' . $slide->file_name)}}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            let status = $('.slide-status').data('value');
            $('.slide-status').val(status);
