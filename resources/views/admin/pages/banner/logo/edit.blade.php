...
@section('main-content-inner')
    <div class="card-header filter-with" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
        <div class="mb-0 ml-1">
            <a href="{{route('logos')}}">List Logo</a>
        </div>
    </div>
    <!-- page title area end -->
            ...
            <div class="col-lg-12 col-ml-12">
                <div class="row">
                    ...
                    <div class="col-12 mt-5">
                        <div class="card">
                            <div class="card-body">
                                <form id="product-form" name="product-form" action="{{ route('logos-update', ['id'=> $logo->id]) }}" method="POST" enctype="multipart/form-data" class="dropzone border-0">
                                    @csrf
                                    @if (session('edit-success'))
                                        <h5 class="logo-message mb-2 text-success">{{ session('edit-success') }}</h5>
                                    @endif
                                    <h4 class="header-title product-add-title">Edit logo</h4>
                                    <div class="form-group">
                                        <label for="product-name" class="col-form-label">Name</label>
                                        <input class="form-control" name="name" type="text" value="{{ $logo->name }}" >
                                    </div>
                                    <div class="form-group">
                                        <label for="services" class="col-form-label">Status</label>
                                        <select class="custom-select logo-status" name="status" data-value="{{ $logo->status }}">
                                                ...
                                                <div class="d-flex">
                                                    <div class="mb-3 mr-2 image-item" style="position: relative">
                                                        ...
                                                        <img width="150px" height="150px" class="logo-img"
                                                             src="{{asset('upload/admin/banner/logo/' . $logo->file_name)}}">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            let status = $('.logo-status').data('value');
            $('.logo-status').val(status);
